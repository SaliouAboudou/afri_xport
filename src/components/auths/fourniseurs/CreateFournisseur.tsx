...
export const  CreateFournisseur = () => {
    ...
    <div className=' w-full bg-amber-50/35 h-screen flex justify-center items-center mx-auto  flex-col flex-wrap'>
          {!showEtapeOne &&
           <div className=' gap:6 mx-5 w-full max-w-sm md:max-w-5xl py-3 flex flex-col md:gap-3 flex-wrap items-center font-bold rounded-t-lg '>
              <h1 className=' text-center text-2xl px-4 py-2 text-gray-600 '>Fournisseur</h1>
              <ul className=' flex justify-center items-center w-full gap-x-4 text-sm font-extralight text-gray-600'>
                ...
                <li className={`flex flex-col gap-y-1.5  ${showEtapeThree ? 'text-amber-600 font-bold' : ''} `}>Informations sur l’entreprise<span><hr className='border-1 rounded-full' /></span></li>
                <li className={`flex flex-col gap-y-1.5  ${showEtapeFour ? 'text-amber-600 font-bold' : ''} `}>Documents légaux<span><hr className='border-1 rounded-full' /></span></li>
              </ul>
            </div>
        }
